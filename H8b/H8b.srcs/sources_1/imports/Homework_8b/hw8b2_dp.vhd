...
begin
	...
	---------------------------------------
	DI_Sig <= std_logic_vector(unsigned(data_in) + to_unsigned(7, data_in'length));
	-- Above line written by ChatGPT with the following prompt:
    -- DI_Sig <= data_in + 7;
    -- I need a way to do this operation essentially, but I can't since data_in currently is a standard_logic_vector
	...
	reset <= '0';							-- BRAM reset is active high, not active low
	...
	sampleMemory: BRAM_SDP_MACRO
		...
		port map (
			...
			RDADDR => std_logic_vector(i_sig),      -- Input address, width defined by port depth
			...
			WRADDR => std_logic_vector(i_sig),		-- Output address, width defined by port depth
			WRCLK => clk,							-- Not writing. Disable.
			WREN => cw(2));			                -- write enable, always writing would be bad
			
	-- what is cw(2) used for? cw_2 is used to determine when to write data_in to the BRAM
